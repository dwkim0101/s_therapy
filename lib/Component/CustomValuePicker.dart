...
class CustomValuePicker extends StatelessWidget {
  ///height of every list element
  static const double DEFAULT_ITEM_EXTENT = 40.0;

  ///width of list view
  static const double DEFAULT_LISTVIEW_WIDTH = 50.0;
    ...
    this.infiniteLoop = false,
      ...
      : assert(initialValue != null),
        ...
        intScrollController = infiniteLoop
          ...
          initialScrollOffset: (initialValue - minValue -1) ~/ step * itemExtent,)
            : new ScrollController(
          initialScrollOffset: (initialValue - minValue) ~/ step * itemExtent,),
        decimalScrollController = null,
        _listViewHeight = 5 * itemExtent,
        integerItemCount = (maxValue - minValue) ~/ step + 1,
        super(key: key);

  ///constructor for decimal number picker
  CustomValuePicker.decimal({
    Key key,
  ...
  int getDistance(int source, int selected) {
    if ((source - selected).abs() == maxValue) return 1;
    if ((source - selected).abs() == maxValue-1) return 2;
    return (source - selected).abs();
  }


  Widget _integerInfiniteListView(ThemeData themeData) {
    TextStyle invisibleStyle = TextStyle(color: Colors.transparent);
    TextStyle farStyle = TextStyle(color: Color(0xffdedede), fontSize: 14);
    TextStyle defaultStyle = TextStyle(color: Color(0xffa8a8a8), fontSize: 20);
    TextStyle selectedStyle = TextStyle(color : Colors.black, fontSize: 20);
      ...
      child: new Container(
        ...
        child: new InfiniteListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            //define special style for selected (middle) element
            TextStyle itemStyle;
            if (getDistance(value, selectedIntValue) == 0) itemStyle = selectedStyle;
            if (getDistance(value, selectedIntValue) == 1) itemStyle = defaultStyle;
            if (getDistance(value, selectedIntValue) == 2) itemStyle = farStyle;
            if (getDistance(value, selectedIntValue) > 2) itemStyle = invisibleStyle;
            return new Center(
              child: new Text(value.toString(), style: itemStyle),
            );
          },
...
class _NumberPickerDialogControllerState extends State<NumberPickerDialog> {
  ...
  CustomValuePicker _buildNumberPicker() {
    if (widget.decimalPlaces > 0) {
      return new CustomValuePicker.decimal(
      ...
      return new CustomValuePicker.integer(
